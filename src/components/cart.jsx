...
const Cart = () => {
  ...
  let cartTotal = useSelector((state) => state.cart.items).reduce(
    (acumulator, cartItem) => acumulator + cartItem.price * cartItem.quantity,
    0
  );
  cartTotal = Math.round(cartTotal * 100) / 100;
  const cartQuantity = useSelector((state) => state.cart.items).reduce(
      ...
      >
        ...
        <CartWrap.ProductsBox>
          {cartItemsList.length === 0 ? (
            ...
            cartItemsList.map((item) => <CartItem key={item.id} item={item} />)
          )}
        </CartWrap.ProductsBox>
        <CartWrap.TotalBox>
